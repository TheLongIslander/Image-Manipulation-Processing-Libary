...
imatrix* init_rgb(imatrix* this, int width, int height)
{
    ...
    {
        // Allocate memory for each row in each channel matrix
        ...
        // Initialize each pixel in each row to zero
...
{  
   // Check if the two matrices have the same dimensions.
    ...
    {
        // Resizing Image 2 to Image 1's width and height
        m2 = resize_image_to_imatrix(m2, m1->width, m1->height);
    }
    // Allocate memory for the output matrix.
    imatrix* result = init_blank_rgb_image(m1->width, m1->height);
    // Add the corresponding elements of the input matrices and store the result in the new imatrix
    for (unsigned short i = 0; i < m1->height; i++)
    {
        for (unsigned short j = 0; j < m1->width; j++)
        {
            result -> r[i][j] =(uint8_t) m1->r[i][j] + m2->r[i][j];
            result -> g[i][j]= (uint8_t) m1->g[i][j] + m2->g[i][j];
            result -> b[i][j]= (uint8_t) m1->b[i][j] + m2->b[i][j];
        }
    }
    // Write the pixel data of the new imatrix to its internal RGB image buffer
...
imatrix* subtract(imatrix* m1, imatrix* m2) 
{
    ...
    // Allocate memory for the output matrix.
    imatrix* result = init_blank_rgb_image(m1->width, m1->height);
    // Subtract the corresponding elements of the input matrices and store the result in the new imatrix
    for (unsigned short i = 0; i < m1->height; i++)
    {
        for (unsigned short j = 0; j < m1->width; j++)
        {
...
imatrix* dot(imatrix* m1, imatrix* m2)
{
    ...
    {
        // Resizing Image 2's width to Image 1's height
        m2 = resize_image_to_imatrix(m2, m1->height, m2->height);
    }
    // Allocate memory for the output matrix.
    imatrix* result = init_blank_rgb_image(m1->height, m2->width);
    // Add the corresponding elements of the input matrices and store the result in the new imatrix
    for(unsigned short i = 0; i < m1->height; i++)
    {
        for(unsigned short j = 0; j < m2 -> width; j++)
        {
            for(unsigned short k = 0; k < m2 -> height; k++)
            {
                result -> r[i][j] += (uint8_t)m1->r[i][j] * m2->r[k][j];
                result -> g[i][j] += (uint8_t)m1->g[i][j] * m2->g[k][j];
                result -> b[i][j] += (uint8_t)m1->b[i][j] * m2->b[k][j];
            }
        }
    }
    // Write the pixel data of the new imatrix to its internal RGB image buffer
...
imatrix* scale(imatrix* this, int width, int height, float alpha)
{
    // Check if the input image or scaling factor are invalid
    if (this == NULL || alpha < 0.0 || alpha > 1.0)
    {
        // Return a reference to the input image without modification
